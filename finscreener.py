...
def getIncomeStatement(sym):
    ...
    if revLen != 0 and cogsLen != 0 and netIncLen != 0:
        ...
        grossMargin = yearlyCOGS[0] / yearlyRevenues[0]
...
def getBalanceSheet(sym):
    ...
    combStat = ((((avgAssetsGrowth - avgLiabilitiesGrowth) * 6)) + avgCashSTInvGrowth + (50 - (avgDebtRatioChange * 5)) + (totalRatio * 15) + (cashRatio * 20)) / 4
...
def combinedStat(incomeStatement, balanceSheet):
    ...
    for x in range(len(incomeStatement)):
        ...
        finalStat = (combGrowthInc * 2.5 + (combGrowthBal * 2)) / 2
        output.append([incomeStatement[x][0], finalStat])
...
for stock in symbols2:
    getIncomeStatement(stock)
    getBalanceSheet(stock)

combinedStat(allStocksCombGrowth, allStocksCashGrowth)
