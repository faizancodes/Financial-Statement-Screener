...
def getIncomeStatement(sym):
    ...
    initialize = True
    dataUnavailable = 'Data not available' in soup

    while dataUnavailable == False and (initialize == True or all([v == 0 for v in yearlyRevenues])):
    ...
    page2 = requests.get('https://finance.yahoo.com/quote/' + sym + '/analysis')
    soup2 = str(BeautifulSoup(page2.text, 'html.parser'))


    for y in range(4):
        if len(soup2[soup2.find('surprisePercent":{') + 37 : soup2.find(',"quarter":{') - 3]) < 100:
            surprises.append(convertNum(soup2[soup2.find('surprisePercent":{') + 35 : soup2.find(',"quarter":{') - 3]))
            soup2 = soup2[soup2.find(',"quarter":{') + 12 : ]
    ...
    if len(surprises) > 0:
        for z in range(len(surprises)): 
            if surprises[z] >= 150:
                ...
                surpriseSum += surprises[z]

        avgSurprise = surpriseSum / len(surprises)
    ...
    combGrowth = ((netGrowth * 8) + avgRevGrowth + (avgSurprise * 1.5) + (avgNetIncGrowth / 2) + (netMargin * 1.5) + grossMargin) / 4
    incomeStatementCombStats.append([sym, combGrowth, netGrowth, avgRevGrowth, avgCOGSGrowth, avgNetIncGrowth, grossMargin, netMargin, avgSurprise])
    ...
    print('Avg Surprise:', avgSurprise)
...
def getBalanceSheet(sym):
    ...
    while dataUnavailable == False and (initialize == True or all([v == 0 for v in yearlyTotalAssets])):
...
def getCashFlow(sym):
    ...
    while dataUnavailable == False and (initialize == True or all([v == 0 for v in yearlyFreeCashFlow])):
...
def writeToCSV(incomeStatement, balanceSheet, cashFlow, final, name):
    ...
    with open('C:\\Users\\faiza\\OneDrive\\Desktop\\StockData\\Finscreener Outputs\\' + name + '.csv', 'w') as f:
        ...
        f.write('Symbol, Final Stat, Inc Comb, BalSheet Comb, CashFlow Comb, Net Growth, Avg Rev Growth, Avg COGS Growth, Avg Net Inc Growth, Gross Margin, Net Margin, Avg Surprise, Avg Assets Growth, Avg Liabilities Growth, Avg Cash&ST Inv Growth, Avg Debt Ratio Change, Total Ratio, Cash Ratio, ROA, Avg FCF Growth, Avg OpCF Growth, FCF Yield' + '\n')

        for x in range(len(incomeStatement)):
            f.write(incomeStatement[x][0] + ',' + str(final[x][1]) + ',' + str(incomeStatement[x][1]) + ',' + str(balanceSheet[x][1]) + ',' + str(cashFlow[x][1]) + ',' + str(incomeStatement[x][2]) + ',' + str(incomeStatement[x][3]) + ',' + str(incomeStatement[x][4]) + ',' + str(incomeStatement[x][5]) + ',' + str(incomeStatement[x][6]) + ',' + str(incomeStatement[x][7]) + ',' + str(incomeStatement[x][8]) + ',' + str(balanceSheet[x][2]) + ',' + str(balanceSheet[x][3]) + ',' + str(balanceSheet[x][4]) + ',' + str(balanceSheet[x][5]) + ',' + str(balanceSheet[x][6]) + ',' + str(balanceSheet[x][7]) + ',' + str(balanceSheet[x][8]) + ',' + str(cashFlow[x][2]) + ',' + str(cashFlow[x][3]) + ',' + str(cashFlow[x][4]) + '\n')
...
for x in range(len(symbols)):
    getIncomeStatement(symbols[x])
    getBalanceSheet(symbols[x])
    getCashFlow(symbols[x])


'''
getIncomeStatement('MKTX')
getBalanceSheet('MKTX')
getCashFlow('MKTX')
'''

combinedStat(incomeStatementCombStats, balanceSheetCombStats, cashFlowCombStats)

writeToCSV(incomeStatementCombStats, balanceSheetCombStats, cashFlowCombStats, finalStats, 'CombStatsOutput5')
